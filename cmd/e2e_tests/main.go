...
func Exec(client *client.Client) {
	Ping(client)
	T_0001(client)
	T_0002(client)
	T_0003(client)
...
func T_0001(client *client.Client) {
	...
	if responseStatusCode != http.StatusBadRequest {
		log.Fatalf(`[T_0001_001] Wallets want status code 400. got code=%d, err=%#v`, responseStatusCode, err.Error())
	}
...
func T_0003(client *client.Client) {
	SetupUserAndWalletCreation(client, "T_0003", "SGD")
}

func T_0004(client *client.Client) {
	username, wallets := SetupUserAndWalletCreation(client, "T_0004", "SGD")
...
func T_0005(client *client.Client) {
	username, wallets := SetupUserAndWalletCreation(client, "T_0005", "SGD")
...
func T_0006(client *client.Client) {
	username, wallets := SetupUserAndWalletCreation(client, "T_0006", "SGD")
...
func T_0007(client *client.Client) {
	username, wallets := SetupUserAndWalletCreation(client, "T_0007", "SGD")

	wallet := wallets[0]

	_, dStatusCode, cErr := client.Deposit(username, wallet.Id, decimal.NewFromFloat(60.2))
	if cErr != nil {
		log.Fatalf(`[T_0007_001] Deposit transaction want nil err, got error %v`, cErr)
	}
	if dStatusCode != http.StatusOK {
		log.Fatalf("[T_0007_001] Deposit want 200. responseStatusCode=%d, err=%v", dStatusCode, cErr)
	}

	wRespBody, wStatusCode, cErr := client.Withdraw(username, wallet.Id, decimal.NewFromFloat(50.1))
	if cErr != nil {
		log.Fatalf(`[T_0007_002] Withdraw transaction want nil err, got error %v`, cErr)
	}
	if wStatusCode != http.StatusOK {
		log.Fatalf("[T_0007_002] Withdraw want 400. responseStatusCode=%d, err=%v", wStatusCode, cErr)
	}
	if wRespBody.Data.Transaction.Id == 0 {
		log.Fatalf(`[T_0007_002] Withdraw want transaction.Id. got 0.`)
	}

	responseBody, walStatusCode, cErr := client.Wallets(username)
	if cErr != nil {
		log.Fatalf("[T_0007_003] Wallets want nil err. responseStatusCode=%d, err=%v", wStatusCode, cErr)
	}
	if walStatusCode != http.StatusOK {
		log.Fatalf("[T_0007_003] Wallets want 400. responseStatusCode=%d, err=%v", wStatusCode, cErr)
	}

	walletAfterTx := responseBody.Data.Wallets[0]
	if walletAfterTx.Balance != "10.1" {
		log.Fatalf(`[T_0007_003] Wallets after transactions want balance=%s. got %s.`, "10.1", walletAfterTx.Balance)
	}

	tRespBody, tStatusCode, cErr := client.Transactions(username)
	if cErr != nil {
		log.Fatalf("[T_0007_004] Transactions want nil err. responseStatusCode=%d, err=%v", tStatusCode, cErr)
	}
	if tStatusCode != http.StatusOK {
		log.Fatalf(`[T_0007_004] Transactions want 200. Got responseStatusCode=%d, err=%#v`, tStatusCode, cErr)
	}
	if len(tRespBody.Data.Transactions) != 2 {
		log.Fatalf(`[T_0007_004] Transactions want transactions.len = 2. got %d`, len(tRespBody.Data.Transactions))
	}
	if tRespBody.Data.Transactions[0].Operation != "withdraw" {
		log.Fatalf(`[T_0007_004] Transactions want transactions[0].operation="withdraw". got %s`, tRespBody.Data.Transactions[0].Operation)
	}
	if tRespBody.Data.Transactions[1].Operation != "deposit" {
		log.Fatalf(`[T_0007_004] Transactions want transactions[0].operation="deposit". got %s`, tRespBody.Data.Transactions[0].Operation)
	}
}

func T_0008(client *client.Client) {
	username0, user0Wallets := SetupUserAndWalletCreation(client, "T_0007", "SGD")
	_, user1Wallets := SetupUserAndWalletCreation(client, "T_0007", "USD")
	...
	if cErr != nil {
		log.Fatalf(`[T_0008_002] Transfer transaction want nil err, got error %v`, cErr)
	}
	if wStatusCode != http.StatusBadRequest {
		log.Fatalf("[T_0008_002] Transfer want 400. responseStatusCode=%d, respBody.Err=%s err=%v", wStatusCode, cErr)
	}

	if wRespBody.Error == nil {
		log.Fatalf(`[T_0008_002] Transfer want non-nil responseBody.Error. got nil`)
	}
	if *wRespBody.Error != "currency_mismatch" {
		log.Fatalf(`[T_0008_002] Transfer want responseBody.Error="currency_mismatch". got %s`, *wRespBody.Error)
	}

	tRespBody, tStatusCode, cErr := client.Transactions(username0)
	if cErr != nil {
		log.Fatalf("[T_0008_003] Transactions want nil err. responseStatusCode=%d, err=%v", tStatusCode, cErr)
	}
	if tStatusCode != http.StatusOK {
	...
	//wRespBody, wStatusCode, cErr := client.Withdraw(username, wallet.Id, decimal.NewFromFloat(50.1))
	//if cErr != nil {
	//	log.Fatalf(`[T_0008_002] Withdraw transaction want nil err, got error %v`, cErr)
	//}
	//if wStatusCode != http.StatusOK {
	//	log.Fatalf("[T_0008_002] Withdraw want 400. responseStatusCode=%d, err=%v", wStatusCode, cErr)
	//}
	//if wRespBody.Data.Transaction.Id == 0 {
	//	log.Fatalf(`[T_0008_002] Withdraw want transaction.Id. got 0.`)
	//}
	//
	//responseBody, walStatusCode, cErr := client.Wallets(username)
	//if cErr != nil {
	//	log.Fatalf("[T_0008_003] Wallets want nil err. responseStatusCode=%d, err=%v", wStatusCode, cErr)
	//}
	//if walStatusCode != http.StatusOK {
	//	log.Fatalf("[T_0008_003] Wallets want 400. responseStatusCode=%d, err=%v", wStatusCode, cErr)
	//}
	//
	//walletAfterTx := responseBody.Data.Wallets[0]
	//if walletAfterTx.Balance != "10.1" {
	//	log.Fatalf(`[T_0008_003] Wallets after transactions want balance=%s. got %s.`, "10.1", walletAfterTx.Balance)
	//}
	//
	//tRespBody, tStatusCode, cErr := client.Transactions(username)
	//if cErr != nil {
	//	log.Fatalf("[T_0008_004] Wallets want nil err. responseStatusCode=%d, err=%v", tStatusCode, cErr)
	//}
	//if tStatusCode != http.StatusOK {
	//	log.Fatalf(`[T_0008_004] Transactions want 200. Got responseStatusCode=%d, err=%#v`, tStatusCode, cErr)
	//}
	//if len(tRespBody.Data.Transactions) != 2 {
	//	log.Fatalf(`[T_0008_004] Transactions want transactions.len > 2. got %d`, len(tRespBody.Data.Transactions))
	//}
	//if tRespBody.Data.Transactions[0].Operation != "withdraw" {
	//	log.Fatalf(`[T_0008_004] Transactions want transactions[0].operation="withdraw". got %s`, tRespBody.Data.Transactions[0].Operation)
	//}
	//if tRespBody.Data.Transactions[1].Operation != "deposit" {
	//	log.Fatalf(`[T_0008_004] Transactions want transactions[0].operation="deposit". got %s`, tRespBody.Data.Transactions[0].Operation)
	//}
}
