...
func T_0001(client *client.Client) {
	...
	if responseData.Error != nil && *responseData.Error != "resource: user not found" {
		log.Fatalf(`[T_0001_001] GetWalletBalances() should fail with Response.error "resource: user not found". got Response.error %s, user id = %d`, *responseData.Error, 0)
	}
...
func T_0002(client *client.Client) {
	...
	responseData, responseStatusCode, err := client.GetTransactionHistory(futureUserName)
	if responseStatusCode != http.StatusBadRequest {
	...
	if responseData.Error != nil && *responseData.Error != "resource: user not found" {
		log.Fatalf(`[T_0002_001] GetTransactionHistory() should fail with Response.error "resource: user not found". got Response.error %s, user id = %d`, *responseData.Error, 0)
	}
	...
	if createUserResponseData.Error != nil && *createUserResponseData.Error != "" {
		log.Fatalf("[T_0002_002] Create User should succeed. createUserResponseData.Error=%v", err)
	}

	if createUserResponseData.Data.Id == 0 {
		log.Fatalf("[T_0002_002] Create User success but not return id. createUserResponseData.Data.Id=%d", createUserResponseData.Data.Id)
	}

	if createUserResponseData.Data.Username != username {
		log.Fatalf("[T_0002_002] Create User success but not return username. createUserResponseData.Data.Username=%s", createUserResponseData.Data.Username)
	}
